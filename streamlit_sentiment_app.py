# streamlit_sentiment_app.py — versi mini compact
...
# ---------- Lexicon ----------
POS = {"bagus","seru","keren","hebat","mantap","menang","gg","top","puas","asik"}
NEG = {"buruk","jelek","lag","noob","toxic","kalah","lemot","sampah","ampas","marah"}
NEU = {"hero","game","rank","ml","tim","build","mode","player"}
...
def hybrid_sentiment(text):
    ...
    for i,w in enumerate(words):
        if w in POS: label="positif"
        elif w in NEG: label="negatif"
        elif w in NEU: label="netral"
        else: label="netral"
...
def main():
    ...
    if not uploaded: return st.info("Silakan upload dataset mentah")
    ...
    with col2:
        ...
        ax.pie(counts,labels=counts.index,autopct="%1.1f%%",colors=["#2ecc71","#f1c40f","#e74c3c"],textprops={"fontsize":7})
        ax.axis("equal")
        st.pyplot(fig)
    ...
    fig_cm,ax_cm=plt.subplots(figsize=(2.8,2))
    ...
    st.subheader("☁️ WordCloud per Sentimen (Mini Size Banget)")
    ...
    for lbl in ["positif","netral","negatif"]:
        ...
        if not teks.strip(): continue
        wc=WordCloud(width=180,height=150,background_color="white",colormap=cmap[lbl]).generate(teks)
        st.markdown(f"**{lbl.capitalize()} (jumlah: {counts.get(lbl,0)})**")
        fig_wc,ax_wc=plt.subplots(figsize=(2,1.5))
