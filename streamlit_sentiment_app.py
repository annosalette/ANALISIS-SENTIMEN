...
lbl = "netral"
if w in POSITIF: lbl = "positif"
elif w in NEGATIF: lbl = "negatif"
elif w in NETRAL: lbl = "netral"
...
if pol > 0.05: tb = "positif"
elif pol < -0.05: tb = "negatif"
else: tb = "netral"
...
with col2:
    ...
    ax.pie(counts, labels=counts.index, autopct="%1.1f%%",
           startangle=90, colors=["#2ecc71","#f1c40f","#e74c3c"],
...
st.subheader("📉 Confusion Matrix (kecil)")
cm = confusion_matrix(y_test, y_pred, labels=["positif","netral","negatif"])
fig_cm, ax = plt.subplots(figsize=(3.2,2.8))
...
for sent in ["positif","netral","negatif"]:
    ...
    wc = WordCloud(width=300, height=200, background_color="white",
                   colormap=cmap[sent], max_words=60,
                   min_font_size=6, max_font_size=28,
                   collocations=False).generate(teks)
    st.markdown(f"**Sentimen: {sent.capitalize()}**")
    fig_wc, ax_wc = plt.subplots(figsize=(2.8,2.0))
